...
tests=(
  ...
  "Complex relationship and WHERE with multiple filters§MATCH (p:Person)-[:KNOWS]->(f1:Person)-[:LIVES_IN]->(f2:City) WHERE f2.name = 'New York' AND p.name = 'Alice' RETURN p.name, f1.name, f2.name§{\"from\":\"Person\",\"relationships\":[{\"type\":\"KNOWS\",\"to\":\"Person\"},{\"type\":\"LIVES_IN\",\"to\":\"City\"}],\"where\":{\"name\":\"Alice\",\"f2.name\":\"New York\"},\"return\":[\"p.name\",\"f1.name\",\"f2.name\"]}"
  ...
  "Relationship with multiple types§MATCH (p:Person)-[:KNOWS|:FRIEND]->(f1:Person) WHERE p.name = 'Alice' RETURN p.name, f1.name§{\"from\":\"Person\",\"relationships\":[{\"type\":\"KNOWS|FRIEND\",\"to\":\"Person\"}],\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\",\"f1.name\"]}"
 "One hop with filter on related node§MATCH (p:Person)-[:KNOWS]->(f1:Person) WHERE f1.age = 30 AND p.name = 'Alice' RETURN p.name, f1.name§{\"from\":\"Person\",\"relationships\":[{\"type\":\"KNOWS\",\"to\":\"Person\"}],\"where\":{\"name\":\"Alice\",\"f1.age\":30},\"return\":[\"p.name\",\"f1.name\"]}"
