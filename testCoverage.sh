...
tests=(
  ...
  "Complex WHERE clause|MATCH (p:Person) WHERE p.name = 'Alice' AND p.age = 30 RETURN p.name, p.age|{\"from\":\"Person\",\"where\":{\"name\":\"Alice\",\"age\":30},\"return\":[\"p.name\",\"p.age\"]}"
  "Multiple conditions in WHERE|MATCH (p:Person) WHERE p.name = 'Alice' AND p.age = 30 AND p.city = 'NYC' RETURN p.name, p.age, p.city|{\"from\":\"Person\",\"where\":{\"name\":\"Alice\",\"age\":30,\"city\":\"NYC\"},\"return\":[\"p.name\",\"p.age\",\"p.city\"]}"
  ...
  "Query with OR condition|MATCH (p:Person) WHERE p.name = 'Alice' OR p.name = 'Bob' RETURN p.name|{\"from\":\"Person\",\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\"]}"
  "Invalid From field (missing node type)|missing 'From' node type|{\"from\":\"\",\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\"]}"
  "Relationship with multiple types|MATCH (p:Person)-[:KNOWS|:FRIEND]->(f:Person) WHERE p.name = 'Alice' RETURN p.name, f.name|{\"from\":\"Person\",\"relationships\":[{\"type\":\"KNOWS|FRIEND\",\"to\":\"Person\"}],\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\",\"f.name\"]}"
  ...
  "Invalid relationship type|missing valid relationship type|{\"from\":\"Person\",\"relationships\":[{\"type\":\"UNKNOWN\",\"to\":\"Person\"}],\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\"]}"
  ...
  "Complex RETURN with dynamic nodes|MATCH (p:Person)-[:KNOWS]->(f:Person)-[:LIVES_IN]->(c:City) WHERE p.name = 'Alice' RETURN p.name, f.name, c.name|{\"from\":\"Person\",\"relationships\":[{\"type\":\"KNOWS\",\"to\":\"Person\"},{\"type\":\"LIVES_IN\",\"to\":\"City\"}],\"where\":{\"name\":\"Alice\"},\"return\":[\"p.name\",\"f.name\",\"c.name\"]}"
